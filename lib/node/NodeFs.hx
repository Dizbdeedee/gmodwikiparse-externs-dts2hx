...
@:jsRequire("node:fs") @valueModuleOnly extern class NodeFs {
	...
	**/
	@:overload(function(filename:node.fs.PathLike, options:Null<Dynamic>, listener:(curr:node.fs.BigIntStats, prev:node.fs.BigIntStats) -> Void):node.fs.StatWatcher { })
	@:overload(function(filename:node.fs.PathLike, listener:(curr:node.fs.Stats, prev:node.fs.Stats) -> Void):node.fs.StatWatcher { })
	static function watchFile(filename:node.fs.PathLike, options:Null<Dynamic>, listener:(curr:node.fs.Stats, prev:node.fs.Stats) -> Void):node.fs.StatWatcher;
	/**
